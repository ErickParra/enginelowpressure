...
if not merged_data.empty:
    ...
    for (name, group), ax in zip(grouped_data, axes.flat):
        ...
        if name in models:
            ...
            ax.plot(X_pressure3, PRESSURE_LVL3_PSI, '--', color='orange', linewidth=0.5, label='Pressure Level 3', markersize=5) 
            ax.plot(X_pressure1, PRESSURE_LVL1_PSI, '--', color='gray', linewidth=0.5,   label='Pressure Level 1', markersize=5) 

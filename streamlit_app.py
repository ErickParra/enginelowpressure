...
# Modelo de regresión y ajuste de curvas
st.write("### Modelos de regresión y ajuste de curvas")
grouped_data = merged_data.groupby('EquipmentName')
 
fig, axes = plt.subplots(nrows=1, ncols=len(grouped_data), figsize=(16, 8))
...
for (name, group), ax in zip(grouped_data, axes.flat):
    ...
    # Aquí deberías ajustar tus modelos de regresión
    # lin_reg, poly_reg = modelos_ajustados_para_cada_equipo(name, X, y)
